...
out:
	...
	if (copy_to_user(user_info, &info, sizeof(info))) {
		SUSFS_LOGE("failed copying back to userspace\n");
	}
	return ret;
}
...
/***************************************/
/* ReSuKiSU compatibility functions    */
/***************************************/
...
void susfs_get_enabled_features(void __user *user_info) {
	...
	if (copy_to_user(user_info, buf, len + 1)) {
...
void susfs_show_variant(void __user *user_info) {
	...
	if (copy_to_user(user_info, variant, strlen(variant) + 1)) {
...
void susfs_show_version(void __user *user_info) {
	...
	if (copy_to_user(user_info, version, strlen(version) + 1)) {
...
int susfs_add_sus_map(void __user *user_info) {
	/* stub - sus_map not available in v1.5.5 */
	SUSFS_LOGI("susfs_add_sus_map called (stub)\n");
	return 0;
}
#endif
